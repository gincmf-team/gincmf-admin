...
const Preview = (props, ref) => {
    ...
    const approve = (s) => {
        const featchData = async () => {
            const result = await approveAduit({
    ...
    const getRender = (item, index) => {
        if (item.type === 'input') {
            return (
                <div key={`com-render-${index}`} className="components bg-white">
        ...
        if (item.type === 'textarea') {
            return (
                <div key={`com-render-${index}`} className="components bg-white" data-index={index}>
                    <div className="label">{item.data.title}：</div>
                    <Input.TextArea
                        disabled
